package com.mykid.platform.common.runner;
...
public class PlatformStartedUpRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        try {
            // 测试 Redis连接是否正常
            redisService.exists("fplatform_test");
        } catch (Exception e) {
            log.error(" ____   __    _   _ ");
            log.error("| |_   / /\\  | | | |");
            log.error("|_|   /_/--\\ |_| |_|__");
            log.error("                        ");
            log.error("Platform启动失败，{}", e.getMessage());
        ...
        if (context.isActive()) {
            ...
            log.info("                                                      ");
            log.info("platform 权限系统启动完毕，地址：{}", url);

            boolean auto = platformProperties.isAutoOpenBrowser();
            String[] autoEnv = platformProperties.getAutoOpenBrowserEnv();
