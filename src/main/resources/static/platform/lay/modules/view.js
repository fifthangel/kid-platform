    ...
    .define(
        ...
        function (exports) {
            ...
            self.loading = function (elem) {
                elem.append(
                    (this.elemLoad = $(
                        '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading platform-loading"></i>'
                    ))
                )
            };
            ...
            self.modal.base = function (msg, params) {
                ...
                params = $.extend(
                    {
                        skin: 'layui-layer-admin-modal platform-alert',
            ...
            self.tab = {
                ...
                minLeft: null,
                maxLeft: null,
                wrap: '.platform-tabs-wrap',
                menu: '.platform-tabs-menu',
                next: '.platform-tabs-next',
                prev: '.platform-tabs-prev',
                step: 200,
                init: function () {
                    var tab = this;
                    var btnCls = tab.wrap + ' .platform-tabs-btn';

                    layui.dropdown.render({
                        elem: '.platform-tabs-down',
                        click: function (name) {
                            if (name === 'all') {
                                tab.delAll();
                            }
                            ...
                            if (name === 'current') {
                                tab.del(layui.platform.route.fileurl);
                            }
                            if (name === 'refresh') {
                                tab.refresh();
                            }
                    ...
                    $(document).on('click', btnCls, function (e) {
                        ...
                        if ($(e.target).hasClass('platform-tabs-close')) {
                            tab.del(url)
                        } else {
                    ...
                    $('.platform-tabs-hidden').addClass('layui-show');
                    this.isInit = true
                },
                has: function (url) {
                    var exists = false;
                ...
                delAll: function (type) {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .platform-tabs-btn';
                ...
                delOther: function () {
                    var tab = this;
                    var menuBtnClas = tab.menu + ' .platform-tabs-btn';
                    $(menuBtnClas + '.platform-tabs-active')
                        .siblings()
                        .each(function () {
                            var url = $(this).attr('lay-url');
                            tab.del(url)
                ...
                del: function (url, backgroundDel) {
                    ...
                    var lay = '[lay-url="' + url + '"]';
                    var thisBody = $(
                        '#' + conf.containerBody + ' > .platform-tabs-item' + lay
                    );
                    ...
                    if (backgroundDel === undefined) {
                        if (thisMenu.hasClass('platform-tabs-active')) {
                            $(this.menu + ' li:last').click()
                        }
                    }
                },
                refresh: function (url) {
                    url = url || layui.platform.route.fileurl;
                    if (this.has(url)) {
                        this.del(url, true);
                        self.renderTabs(url)
                    }
                ...
                clear: function () {
                    this.data = [];
                    this.isInit = false;
                    $(document).off('click', this.wrap + ' .platform-tabs-btn')
                },
                change: function (route, callback) {
                    ...
                    var activeCls = 'platform-tabs-active';
                    ...
                    if (existsTab) {
                        ...
                        layui.platform.navigate(route.href)
                    } else {
                        self.loadHtml(fileurl, function (res) {
                            var htmlElem = $(
                                "<div><div class='platform-tabs-item' lay-url='" +
                                fileurl +
                                "'>" +
                                res.html +
                                '</div></div>'
                            ...
                            tab.data.push(route);
                            layui.platform.render(tab.tabMenuTplId);
                    ...
                    layui.platform.sidebarFocus(route.href);
                    return false
                },
                onChange: function () {
                }
            ...
            self.renderLayout = function (callback, url) {
                ...
                self.render(url, function (res) {
                    ...
                    if (conf.viewTabs === true) {
                        self.containerBody.addClass('platform-tabs-body')
                    }
                    layui.platform.appBody = self.containerBody;
